...
void Dialog::dosyaOku()
{
    ...
    int y;
    QList<int> konumlar;
    QFile file("D:\\GorselProgramlama\\21100011050\\konumlar.txt"); //okunacak dosya
...
{
    skorlar.clear();
    QFile file("D:\\GorselProgramlama\\21100011050\\skorlar.txt"); //okunacak dosya
    if(!file.open(QIODevice::ReadOnly)) { //dosya okunamıyorsa hata mesajı veriyor.
    QMessageBox::information(0, "error", file.errorString());
    }
    QTextStream in(&file);
...
QFile writeFile("D:\\GorselProgramlama\\21100011050\\skorlar.txt"); //yazılacak dosya
